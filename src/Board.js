...
function Board(props) {
  ...
  useLayoutEffect(() => {
    async function fetchData() {
      fetch("http://localhost:3333/data", {
        method:"POST",
        body: JSON.stringify({type:"read",id:props.id}),
        headers: {
  ...
  function postData(datai) {
    ...
    fetch("http://localhost:3333/data", {
      method: "POST",
      body: JSON.stringify({id:props.id,data:postData,type:"write"}),
      headers: {
        // Добавляем необходимые заголовки
        "Content-type": "application/json; charset=UTF-8",
      },
  ...
  return (
    //TODO:Handle adding
      ...
      <><Menu page = {props.id}></Menu>
      <div
      className="Board"
      style={{
        width: Math.max(columns.length * 300 + 300, window.screen.availWidth),
        height: "inherit",
        overflow: "hidden",
      }}
    >
